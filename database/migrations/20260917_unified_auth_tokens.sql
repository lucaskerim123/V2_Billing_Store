-- Store-owned authentication tokens. Supabase Auth is not used as the application identity store.
create table if not exists public.orbitfs_email_verification_tokens (
  ...
  request_ip inet, created_at timestamptz not null default now()
);
...
create table if not exists public.orbitfs_password_reset_tokens (
  id uuid primary key default gen_random_uuid(), user_id uuid not null references public.users(id) on delete cascade,
  token_hash text not null unique, expires_at timestamptz not null, used_at timestamptz,
  request_ip inet, created_at timestamptz not null default now(), requested_by uuid references public.users(id) on delete set null
);
