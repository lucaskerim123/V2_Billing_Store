-- Store-owned authentication tokens. Supabase Auth is infrastructure only.
create table if not exists public.orbitfs_email_verification_tokens (
 id uuid primary key default gen_random_uuid(), user_id uuid not null references public.users(id) on delete cascade, token_hash text not null unique,
 expires_at timestamptz not null, used_at timestamptz, request_ip inet, created_at timestamptz not null default now()
);
...
create table if not exists public.orbitfs_password_reset_tokens (
 id uuid primary key default gen_random_uuid(), user_id uuid not null references public.users(id) on delete cascade, token_hash text not null unique,
 expires_at timestamptz not null, used_at timestamptz, request_ip inet, created_at timestamptz not null default now(), requested_by uuid references public.users(id) on delete set null
);
...
values ('account.email_verification','Account email verification','account','Send the OrbitFS account verification link.','account.email_verification',true,'["customer_name","verification_url","expires_hours"]'::jsonb),('account.password_reset','Password reset','account','Send the OrbitFS password reset link.','account_password_reset',true,'["customer_name","reset_url","expires_minutes"]'::jsonb)
