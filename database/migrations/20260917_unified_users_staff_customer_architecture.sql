...
create table if not exists public.users (
 id uuid primary key default gen_random_uuid(), email text not null, username text, display_name text, first_name text, last_name text,
...
create table if not exists public.staff_access (
 user_id uuid primary key references public.users(id) on delete cascade, enabled boolean not null default true, role text not null default 'staff', permissions jsonb not null default '{}'::jsonb,
