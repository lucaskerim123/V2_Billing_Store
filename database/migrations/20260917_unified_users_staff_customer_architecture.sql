...
select coalesce(c.auth_user_id,p.id,gen_random_uuid()),lower(coalesce(c.email,'unknown+'||coalesce(c.auth_user_id::text,p.id::text,gen_random_uuid()::text)||'@invalid.local')),c.username,coalesce(c.display_name,c.name,p.display_name),coalesce(c.first_name,p.first_name),coalesce(c.last_name,p.last_name),case when coalesce(c.status,p.status,'active') in ('active','suspended','disabled','pending') then coalesce(c.status,p.status,'active') else 'active' end,coalesce(c.email_verified_at,p.email_verified_at),coalesce(c.created_at,p.created_at,now()),now() from public.customers c full join public.user_profiles p on p.id=c.auth_user_id where not exists(select 1 from public.users u where u.id=coalesce(c.auth_user_id,p.id)) and c.email is not null;
