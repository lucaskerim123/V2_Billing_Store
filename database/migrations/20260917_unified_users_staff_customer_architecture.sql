...
-- Existing customer identity is the source for email; user_profiles contributes staff/profile data.
...
select coalesce(c.auth_user_id,p.id,gen_random_uuid()),
       lower(coalesce(c.email,'unknown+'||coalesce(c.auth_user_id::text,p.id::text,gen_random_uuid()::text)||'@invalid.local')),
       coalesce(c.username),
       coalesce(c.display_name,c.name,p.display_name),
       coalesce(c.first_name,p.first_name),
       coalesce(c.last_name,p.last_name),
       ...
       coalesce(c.created_at,p.created_at,now()),now()
...
set email=lower(coalesce(nullif(u.email,''),c.email,u.email)),
    ...
    last_name=coalesce(u.last_name,c.last_name,p.last_name),
...
-- Profiles that exist only as staff users need an email from the legacy Auth identity during the transition.
...
select p.id,lower(coalesce(a.email,'staff+'||p.id::text||'@invalid.local')),p.display_name,p.first_name,p.last_name,
       case when p.status in ('active','suspended','disabled','pending') then p.status else 'active' end,
       p.email_verified_at,coalesce(p.created_at,now()),now()
