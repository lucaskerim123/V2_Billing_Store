...
create table if not exists public.users (
  id uuid primary key default gen_random_uuid(), email text not null, username text,
  display_name text, first_name text, last_name text, status text not null default 'active',
...
create table if not exists public.staff_access (
  user_id uuid primary key references public.users(id) on delete cascade, enabled boolean not null default true,
...
select coalesce(c.auth_user_id,p.id,gen_random_uuid()), lower(coalesce(c.email,'unknown+'||coalesce(c.auth_user_id::text,p.id::text,gen_random_uuid()::text)||'@invalid.local')),
       c.username,coalesce(c.display_name,c.name,p.display_name),coalesce(c.first_name,p.first_name),coalesce(c.last_name,p.last_name),
       ...
       coalesce(c.email_verified_at,p.email_verified_at),coalesce(c.created_at,p.created_at,now()),now()
...
select p.id,lower(coalesce(a.email,'staff+'||p.id::text||'@invalid.local')),p.display_name,p.first_name,p.last_name,
       case when p.status in ('active','suspended','disabled','pending') then p.status else 'active' end,p.email_verified_at,coalesce(p.created_at,now()),now()
from public.user_profiles p left join auth.users a on a.id=p.id
...
select p.id,true,case when lower(coalesce(p.role,'')) in ('owner','system_admin','admin','superadmin','senior_support','staff') then lower(p.role) else 'staff' end,'{}'::jsonb
from public.user_profiles p where lower(coalesce(p.role,'')) in ('owner','system_admin','admin','superadmin','senior_support','staff')
