...
const compact=(output)=>{const lines=String(output||"").split(/\r?\n/).map(x=>x.trimEnd()).filter(Boolean);let i=-1;for(let n=lines.length-1;n>=0;n--){if(/##\[error\]/i.test(lines[n])){i=n;break;}}if(i<0)for(let n=lines.length-1;n>=0;n--){if(/(?:npm ERR!|Error:|error TS\d+|Type error|Build failed|failed with|Expected .+ got)/i.test(lines[n])){i=n;break;}}if(i<0)return ["No concise error line was found in command output."];return lines.slice(Math.max(0,i-5),i+1).map(x=>x.replace(/^.*?##\[error\]\s*/,"").trim()).filter(Boolean);};
