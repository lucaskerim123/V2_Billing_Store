...
export default function LicenseControllerPage(){
  ...
  async function request(path:string,method:"GET"|"POST"="GET",body?:any){
    ...
    const response=await fetch(`/api/admin/license-master?path=${encodeURIComponent(path)}`,{
      method,
  ...
  async function load(){
    ...
    }catch(e){setError(e instanceof Error?e.message:"Failed to load licences");}
  ...
  async function control(id:string,action:string){
    ...
    try{
      await request(`/api/license/${encodeURIComponent(id)}/control`,"POST",{action,actorRef:"billing_store_license_controller"});
      await load();
    }catch(e){setError(e instanceof Error?e.message:"License control request failed");}
    finally{setBusy("");}
  }
  ...
  return <main style={{padding:24,maxWidth:1250,margin:"0 auto"}}>
    <div style={{display:"flex",justifyContent:"space-between",gap:16,alignItems:"center",marginBottom:20}}>
      <div><div style={{fontSize:12,letterSpacing:1,textTransform:"uppercase",opacity:.65}}>My OrbitFS · Licensing</div><h1 style={{margin:"6px 0"}}>License Controller</h1><p style={{margin:0,opacity:.75}}>Billing Store is the control surface. License Master remains the licensing authority.</p></div>
    ...
    {loading?<p>Loading licences…</p>:!licenses.length?<div style={{padding:20,border:"1px solid #ddd",borderRadius:10}}>No licences have been provisioned yet.</div>:<div style={{display:"grid",gap:12}}>{licenses.map((license)=><section key={String(license.id)} style={{padding:16,border:"1px solid #ddd",borderRadius:10}}>
      <div style={{display:"flex",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}>
        <div><strong>{String(license.product_code||"orbitfs_base")}</strong><div style={{fontSize:13,opacity:.7}}>License {String(license.id||"")}</div></div>
        <strong>{String(license.status||"unknown")}</strong>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:10,marginTop:14,fontSize:13}}>
        <div><span>Customer</span><br/><strong>{String(license.customer_ref||"—")}</strong></div>
        <div><span>Order</span><br/><strong>{String(license.order_ref||"—")}</strong></div>
