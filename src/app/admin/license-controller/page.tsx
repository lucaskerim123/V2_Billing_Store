...
export default function LicenseControllerPage(){
  ...
  return <main className="lmPage licenseControllerPage">
    ...
    {error&&<div className="lmCard" role="alert"><b>License Master request failed</b><p className="muted">{error}</p></div>}{newKey&&<section className="lmCard" role="status" style={{marginBottom:14}}><div className="lmKicker">NEW LICENSE KEY</div><h2>Replacement key generated</h2><p className="muted">This plaintext key is displayed once. Copy/save it now. License Master does not store plaintext keys.</p><code style={{display:"block",wordBreak:"break-all",fontSize:"1.05rem",padding:"12px 14px",border:"1px solid currentColor",borderRadius:8}}>{newKey}</code><div className="lmHeroActions" style={{marginTop:12}}><button onClick={()=>void navigator.clipboard?.writeText(newKey)}>Copy new key</button><button onClick={()=>setNewKey("")}>Dismiss</button></div></section>}
    {loading?<div className="lmCard lmLoading">Loading licences…</div>:!licenses.length?<div className="lmCard"><div className="lmKicker">LICENSING</div><h2>No licences</h2><p className="muted">No licences have been provisioned yet.</p></div>:<section className="lmCard"><div className="lmKicker">LICENSE MASTER RECORDS</div><h2>Licences</h2><p className="lmHint">Customer ID is the Billing Store customer reference.</p><div className="lmReleaseList">{licenses.map((license)=><article className="lmRelease" key={String(license.id)}>
      ...
      <div className="lmHeroActions" style={{marginTop:12}}>{["rotate","activate","suspend","terminate","unlock"].map(action=><button key={action} disabled={!!busy||String(license.status)==="terminated"&&action!=="unlock"&&action!=="rotate"} onClick={()=>void control(String(license.id),action)}>{busy===`${license.id}:${action}`?"Working…":actionLabels[action]}</button>)}</div>
    </article>)}</div></section>}
  </main>;
}