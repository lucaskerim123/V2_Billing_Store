...
export default function LicenseControllerPage(){
  ...
  return <main className="lmPage">
    ...
    {error&&<div className="lmCard" role="alert"><b>License Master request failed</b><p className="muted">{error}</p></div>}
    {loading?<div className="lmCard lmLoading">Loading licences…</div>:!licenses.length?<div className="lmCard"><div className="lmKicker">LICENSING</div><h2>No licences</h2><p className="muted">No licences have been provisioned yet.</p></div>:<section className="lmCard"><div className="lmKicker">LICENSE MASTER RECORDS</div><h2>Licences</h2><div className="lmReleaseList">{licenses.map((license)=><article className="lmRelease" key={String(license.id)}>
