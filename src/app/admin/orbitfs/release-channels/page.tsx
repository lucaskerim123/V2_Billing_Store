...
export default function ReleaseChannelsAdmin(){
 ...
 return <main className="lmPage">
  <div className="lmHero"><div><div className="lmEyebrow">ORBITFS · CUSTOMER DELIVERY</div><h1>Release Channels</h1><p>License Master owns Stable, Beta and Development. Billing Store controls only which customers receive those channels.</p></div><div style={{display:"flex",gap:8,flexWrap:"wrap"}}><a className="buttonlink" href="/admin/orbitfs">← My OrbitFS</a><button onClick={()=>void load()} disabled={!!busy}>{busy==="load"?"Refreshing…":"Refresh"}</button></div></div>
