...
export default function OrbitFSUpdateReleaseDeployer(){
 ...
 const [systemSettings,setSystemSettings]=useState<any>(null),[savingSetting,setSavingSetting]=useState("");
 ...
 async function load(){
  ...
  try{
   ...
   setReleases(rows);setSelected(current=>rows.some((r:any)=>r.id===current)?current:(rows[0]?.id||""));const [cr,sr]=await Promise.all([fetch("/api/admin/orbitfs/release-channels",{headers:auth,cache:"no-store"}),fetch("/api/admin/orbitfs/deployment-settings",{headers:auth,cache:"no-store"})]);const cj=await cr.json().catch(()=>({})),sj=await sr.json().catch(()=>({}));if(!sr.ok)throw Error(sj.error||"Could not load customer update controls");setChannels((cj.channels||[]).filter((x:any)=>x.enabled&&x.customer_visible));setSystemSettings(sj.settings||{});
 ...
 async function setDeploymentSetting(key:"enabled"|"maintenance_mode"|"customer_updates_enabled",value:boolean){setSavingSetting(key);setMsg("");try{const auth=await adminHeaders();const r=await fetch("/api/admin/orbitfs/deployment-settings",{method:"PATCH",headers:{...auth,"content-type":"application/json"},body:JSON.stringify({[key]:value})}),j=await r.json().catch(()=>({}));if(!r.ok)throw Error(j.error||"Could not update customer update controls");setSystemSettings(j.settings||{});setMsg(key==="maintenance_mode"?(value?"Customer deployment maintenance mode enabled.":"Customer deployment maintenance mode disabled."):"Customer update access updated.")}catch(e:any){setMsg(e?.message||"Could not update customer update controls")}finally{setSavingSetting("")}}
 async function editMaintenanceMessage(){const value=prompt("Customer maintenance notice",String(systemSettings?.maintenance_message||"OrbitFS deployment services are temporarily unavailable while maintenance is in progress."))?.trim();if(!value)return;setSavingSetting("maintenance_message");setMsg("");try{const auth=await adminHeaders();const r=await fetch("/api/admin/orbitfs/deployment-settings",{method:"PATCH",headers:{...auth,"content-type":"application/json"},body:JSON.stringify({maintenance_message:value})}),j=await r.json().catch(()=>({}));if(!r.ok)throw Error(j.error||"Could not update maintenance notice");setSystemSettings(j.settings||{});setMsg("Maintenance notice updated.")}catch(e:any){setMsg(e?.message||"Could not update maintenance notice")}finally{setSavingSetting("")}}
 ...
 return <main className="orbitfsControlPage">
  ...
  <section className="orbitfsCard" style={{marginTop:12}}>
   <div className="orbitfsCardHeader"><div><div className="orbitfsKicker">CUSTOMER UPDATE ACCESS</div><h2>Update execution controls</h2><p className="orbitfsMuted">Controls whether customers can apply published Update releases. License Manager remains the technical authority.</p></div></div>
   <div className="orbitfsAuthority">
    <div className="orbitfsAuthorityRow"><b>Customer deployment system</b><span>{systemSettings?.enabled===false?"Disabled":"Enabled"}</span><button type="button" disabled={!systemSettings||!!savingSetting} onClick={()=>void setDeploymentSetting("enabled",systemSettings?.enabled===false)}>{savingSetting==="enabled"?"Saving…":systemSettings?.enabled===false?"Enable":"Disable"}</button></div>
    <div className="orbitfsAuthorityRow"><b>Maintenance mode</b><span>{systemSettings?.maintenance_mode?"Active":"Normal"}{systemSettings?.maintenance_message?` · ${systemSettings.maintenance_message}`:""}</span><div style={{display:"flex",gap:8,flexWrap:"wrap"}}><button type="button" disabled={!systemSettings||!!savingSetting} onClick={()=>void editMaintenanceMessage()}>{savingSetting==="maintenance_message"?"Saving…":"Edit notice"}</button><button type="button" disabled={!systemSettings||!!savingSetting} onClick={()=>void setDeploymentSetting("maintenance_mode",!systemSettings?.maintenance_mode)}>{savingSetting==="maintenance_mode"?"Saving…":systemSettings?.maintenance_mode?"Disable maintenance":"Enable maintenance"}</button></div></div>
    <div className="orbitfsAuthorityRow"><b>Customer Update deployment</b><span>{systemSettings?.customer_updates_enabled===false?"Disabled":"Enabled"}</span><button type="button" disabled={!systemSettings||!!savingSetting} onClick={()=>void setDeploymentSetting("customer_updates_enabled",systemSettings?.customer_updates_enabled===false)}>{savingSetting==="customer_updates_enabled"?"Saving…":systemSettings?.customer_updates_enabled===false?"Enable":"Disable"}</button></div>
   </div>
  </section>
