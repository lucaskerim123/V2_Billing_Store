...
export default function LicenseMasterSettings(){
  ...
  return <main className="adminShell settingsCompact">
    ...
    <section className="panel"><h2>Authority</h2><div className="form"><label>Master API<input readOnly value={data?.configuredUrl||"https://incendiarynetworks.cc/api"}/></label><label>Master Admin Panel<input readOnly value="https://panel.incendiarynetworks.cc"/></label><label>Status<input readOnly value={c?.enabled?"Enabled":"Disabled"}/></label><label>Last successful test<input readOnly value={c?.last_success_at?new Date(c.last_success_at).toLocaleString():"Not tested"}/></label><label>Last error<input readOnly value={c?.last_error||"None"}/></label></div></section>
    <section className="panel"><h2>API contract</h2><div className="form"><label>Health<input readOnly value="/api/health"/></label><label>Issue licence<input readOnly value="/api/license/issue"/></label><label>Validate licence<input readOnly value="/api/license/validate"/></label><label>Control licence<input readOnly value="/api/license/{id}/control"/></label></div></section>
