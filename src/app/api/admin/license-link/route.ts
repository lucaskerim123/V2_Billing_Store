...
function matchReason(x:any,c:any,orders:any[]){
  ...
  const refs=new Set((orders||[]).flatMap((o:any)=>[o.id,o.order_number]).filter(Boolean).map(normalize));
  const m=x?.metadata||{};
  const vals=[x?.customer_external_id,x?.customer_ref,x?.external_customer_id,x?.customer_id, m.customerNumber,m.customerId,m.customerRecordId,m.customer_external_id,x?.external_reference,x?.order_ref,m.orderRef,m.order_number].map(normalize).filter(Boolean);
  ...
  if(email&&[normalize(x?.customer_email),normalize(x?.email),normalize(m.customerEmail),normalize(m.email)].includes(email))return "email";
...
export async function GET(req:Request){
 try{
  ...
  const customerId=String(new URL(req.url).searchParams.get("customerId")||"").trim();if(!customerId)return Response.json({error:"Customer is required"},{status:400});
  ...
  return Response.json({customer,bindings,licenses:all.filter((x:any)=>x._linked||x._match),allLicenses:all});
 }catch(e:any){return Response.json({error:e?.message||"Could not load license assignments"},{status:e?.status||500});}
}
export async function POST(req:Request){
 try{
  const actor=await staff(req);if(!actor)return Response.json({error:"License management permission required"},{status:403});
  const body=await req.json().catch(()=>({})),customerId=String(body.customerId||"").trim(),wanted=String(body.licenseId||"").trim(),mode=String(body.mode||"manual").trim();
  if(!customerId)return Response.json({error:"Customer is required"},{status:400});
  const {data:customer,error}=await actor.sb.from("customers").select("id,auth_user_id,user_id,customer_number,name,email").eq("id",customerId).maybeSingle();
  if(error)throw error;if(!customer)return Response.json({error:"Customer not found"},{status:404});
  const userId=String(customer.auth_user_id||customer.user_id||"").trim();if(!userId)return Response.json({error:"Customer has no linked account"},{status:400});
  const [orders,m]=await Promise.all([actor.sb.from("orders").select("id,order_number").eq("auth_user_id",userId),masterLicenses().catch(()=>({licenses:[]}))]);
  const master=Array.isArray(m?.licenses)?m.licenses:[];let selected:any=null,reason="";
  if(mode==="auto"){
   const matches=master.map((x:any)=>({...x,_match:matchReason(x,customer,orders.data||[])})).filter((x:any)=>x._match);
   const strong=matches.filter((x:any)=>["customer number","customer record","account id"].includes(x._match));
   selected=strong.length===1?strong[0]:matches.length===1?matches[0]:null;reason=selected?selected._match:"";
  ...
  const existing=await db.from("license_bindings").select("id").eq("auth_user_id",userId).eq("license_id",selectedId).maybeSingle();if(existing.error)throw existing.error;
