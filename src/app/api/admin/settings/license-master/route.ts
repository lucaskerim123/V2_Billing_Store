...
export async function GET(req:Request){
  try{
    ...
    return Response.json({connection:data||null,configuredUrl:data?.master_url||process.env.MASTER_API_URL||MASTER_URL,healthPath:"/api/health",productsPath:"/api/products",licenseIssuePath:"/api/license/issue",licenseValidatePath:"/api/license/validate",licenseListPath:"/api/licenses",licenseControlPath:"/api/license/{id}/control"},{headers:{"cache-control":"no-store"}});
...
export async function POST(req:Request){
  try{
    ...
    try{
      ...
      if(row)await db.from("license_master_connection").update({master_url:MASTER_URL,enabled:true,last_tested_at:now,last_success_at:now,last_error:null,updated_at:now}).eq("id",row.id);
      else await db.from("license_master_connection").insert({master_url:MASTER_URL,enabled:true,last_tested_at:now,last_success_at:now,last_error:null});
      ...
      const {data:row}=await db.from("license_master_connection").select("id").order("updated_at",{ascending:false}).limit(1).maybeSingle();
      if(row)await db.from("license_master_connection").update({master_url:MASTER_URL,last_tested_at:now,last_error:message,updated_at:now}).eq("id",row.id);
