...
async function currentUser(req:Request){
  ...
  try{
    const result:any=await sb.auth.getUser(token);
...
export async function GET(req:Request){
  try{
    ...
    return Response.json(await masterRequest("/api/orbitfs/status",{method:"GET",headers:{"x-orbit-user-id":user.id}},"billing"),{headers:{"cache-control":"no-store"}});
  }catch(e:any){
