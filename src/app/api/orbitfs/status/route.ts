...
export async function GET(req:Request){
 try{
  ...
  const [bindings,connections,installations,settings,bundles,masterLicenseResult,masterBaseResult,masterUpdateResult,customerChannelsResult]=await Promise.all([
   ...
   masterReleases("orbitfs_base","stable","base").catch(()=>({releases:[]})),
   masterReleases("orbitfs_base","stable","update").catch(()=>({releases:[]}))
  ]);
  const installationRows=installations.data||[],bindingRows=bindings.data||[],masterLicensesRows=masterLicenseResult?.licenses||[],masterBaseRows=masterBaseResult?.releases||[],masterUpdateRows=masterUpdateResult?.releases||[],masterReleaseRows=[...masterBaseRows,...masterUpdateRows];
  ...
  return Response.json({settings:{enabled:s.enabled!==false,customer_deploy_enabled:s.customer_deploy_enabled!==false,customer_updates_enabled:s.customer_updates_enabled!==false,customer_rollbacks_enabled:s.customer_rollbacks_enabled!==false,supabase_oauth_enabled:s.supabase_oauth_enabled!==false,vercel_oauth_enabled:s.vercel_oauth_enabled!==false,allow_existing_supabase_project:s.allow_existing_supabase_project!==false,allow_create_supabase_project:s.allow_create_supabase_project!==false,schema_version:s.schema_version||"1",release_channel:s.release_channel||"stable"},bindings:enrichedBindings,connections:connectionRows,installations:installationRows,events:eventRows.data||[],releases:installReleaseRows.data||[],publishedReleases:publishedMaster,latestRelease:latestUpdate||latestBase,latestBase,latestUpdate,master:{licenses:masterLicensesRows,releases:masterReleaseRows}},{headers:{"cache-control":"no-store"}});
