...
const FORBIDDEN_MASTER_HOSTS = new Set(["api.incendiarynetworks.cc"]);

function assertMasterApiUrl(value: string) {
  const u = new URL(value);
  if (u.protocol !== "https:" || u.search || u.hash || u.pathname.replace(/\\/+$/, "") !== "/api" || FORBIDDEN_MASTER_HOSTS.has(u.hostname.toLowerCase())) {
    throw new Error("License Master API URL must be an HTTPS /api endpoint and api.incendiarynetworks.cc is no longer supported");
  }
}

async function configuredMasterApiBase() {
  try {
    const {data} = await licenseDb()
      .from("license_master_connection")
      .select("master_url,enabled")
...
export async function masterLicenseValidate(input:any){
  ...
  if(!ALLOWED_PRODUCTS.has(product)) throw Object.assign(new Error("Unsupported OrbitFS license product"),{status:400,code:"UNSUPPORTED_PRODUCT"});
  if(!licenseKey) throw Object.assign(new Error("License key is required"),{status:400,code:"LICENSE_KEY_REQUIRED"});
  return masterRequest("/api/v1/licenses/validate",{method:"POST",body:JSON.stringify({
