...
export async function masterHealth(){const response=await fetch("https://incendiarynetworks.cc/api/v1/license/health",{method:"GET",cache:"no-store",headers:{accept:"application/json"}});const data=await response.json().catch(()=>({}));if(!response.ok)throw Object.assign(new Error(data?.error||data?.code||`License Master health check failed (${response.status})`),{status:response.status,code:data?.code});return data;}\nexport const masterProducts=()=>masterRequest("/api/v1/products",{method:"GET"});
