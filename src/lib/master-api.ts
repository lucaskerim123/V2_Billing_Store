...
function masterPath(path:string){const clean=path.startsWith("/")?path:`/${path}`;return clean.startsWith("/api/v1/")?clean.slice(8):clean.startsWith("/api/")?clean.slice(4):clean;}
