...
export type OrbitUser={
 id:string; email:string; username:string|null; displayName:string; status:string;
 customerId:string|null; staff:boolean; staffRole:string|null; permissions:Record<string,boolean>;
};
...
export async function createOrbitSession(userId:string,request?:Request){
 ...
 const {error}=await db().from("customer_sessions").insert({
  user_id:userId,canonical_user_id:userId,token_hash:hashToken(raw),expires_at:expires.toISOString(),
...
export async function clearOrbitSession(){
 ...
 if(token)await db().from("customer_sessions").update({revoked_at:new Date().toISOString()}).eq("token_hash",hashToken(token)).is("revoked_at",null);
...
export async function authenticateOrbitUser(email:string,password:string){
 const client=db(),normalized=email.trim().toLowerCase();
 const {data:user}=await client.from("users").select("id,email,username,display_name,first_name,status,email_verified_at").ilike("email",normalized).maybeSingle();
 if(!user||user.status!=="active")return null;
 const {data:credential}=await client.from("customer_credentials").select("password_hash").eq("user_id",user.id).maybeSingle();
 if(!credential?.password_hash||!verifyCustomerPassword(password,String(credential.password_hash)))return null;
 return user;
}

export async function setOrbitPassword(userId:string,password:string){
 const now=new Date().toISOString(),client=db(),passwordHash=hashCustomerPassword(password);
 const {error}=await client.from("customer_credentials").upsert({user_id:userId,canonical_user_id:userId,password_hash:passwordHash,password_changed_at:now,updated_at:now},{onConflict:"user_id"});
 if(error)throw error;
 await client.from("customer_sessions").update({revoked_at:now}).eq("user_id",userId).is("revoked_at",null);
}
...
export async function requireOrbitStaff(){
 const user=await requireOrbitUser();
 if(!user.staff)throw Object.assign(new Error("Staff access required."),{status:403});
 return user;
}
