...
async function publishedRelease(version:string|undefined,action:DeployAction):Promise<any>{const rows=await masterReleases("orbitfs_base","stable",releaseType(action));const releases=(Array.isArray(rows?.releases)?rows.releases:Array.isArray(rows)?rows:[]).filter((r:any)=>String(r.status||"").toLowerCase()==="published"&&String(r.review_status||"").toLowerCase()==="approved");const wanted=version?releases.find((r:any)=>String(r.version)===version):releases[0];if(!wanted?.id)fail(version?`Published ${releaseType(action)} release ${version} was not found in License Master`:`No approved published ${releaseType(action)} release is available`,404);return wanted}

function gunzipArtifact(bytes:Buffer):Buffer{try{return gunzipSync(bytes)}catch{fail("Release artifact is not a valid OrbitFS gzip package",422)}}
function parsePackage(raw:Buffer):Package{try{const value=JSON.parse(raw.toString("utf8")) as Package;if(!value||!Array.isArray(value.files)||!value.version)fail("Release package manifest is incomplete",422);return value}catch(error){if(error instanceof Error&&"status" in error)throw error;fail("Release package contains invalid JSON",422)}}
