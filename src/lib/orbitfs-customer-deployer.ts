...
async function previousDeployment(install:any){const {data,error}=await licenseDb().from("orbitfs_installation_releases").select("vercel_deployment_id,release_version,release_id,created_at").eq("installation_id",install.id).eq("status","ready").order("created_at",{ascending:false}).limit(2);if(error)throw error;return (data||[]).find((r:any)=>r.vercel_deployment_id!==install.vercel_deployment_id)||null}
...
export async function runCustomerDeployer(install:any,action:DeployAction,version?:string){
  ...
  if(action==="rollback"){
    const previous=await previousDeployment(install);if(!previous?.vercel_deployment_id||!previous.release_id)fail("No previous successful deployment is available for rollback",409);
    await masterExecuteDeployment({action:"rollback",releaseId:previous.release_id,installationId:install.id,userRef:install.auth_user_id});
    await event(install,"deployment.rollback.started","info",`Rolling back to ${previous.release_version}`,{deploymentId:previous.vercel_deployment_id});
    const result=await vercelApi(install.auth_user_id,`/v9/projects/${encodeURIComponent(install.vercel_project_id)}/rollback/${encodeURIComponent(previous.vercel_deployment_id)}`,{method:"POST",body:JSON.stringify({})});
    const {data,error}=await licenseDb().from("orbitfs_installations").update({previous_release_version:install.release_version||null,release_version:previous.release_version,release_id:previous.release_id,vercel_deployment_id:previous.vercel_deployment_id,last_deployment_at:new Date().toISOString(),last_error:null,state:"deployed"}).eq("id",install.id).select().single();
    if(error)throw error;await event(data,"deployment.rollback.completed","ok",`Rolled back to ${previous.release_version}`,{deploymentId:previous.vercel_deployment_id,result});return data;
  }
