...
export async function configureVercel(install:any,releaseVersion?:string,panelUrl?:string,releaseChannel?:string,releaseId?:string,releaseSha256?:string,releaseSourceCommit?:string){const key=await publishableKey(install),secret=await installationSecret(install.id,"db_secret");if(!secret)throw new Error("OrbitFS database secret is missing");const vars:Record<string,string>={SUPABASE_URL:`https://${install.supabase_project_ref}.supabase.co`,SUPABASE_PUBLISHABLE_KEY:key,ORBITFS_DB_SECRET:secret,ORBITFS_PANEL_URL:panelUrl||"https://panel.incendiarynetworks.cc",ORBITFS_LICENSE_API_URL:"https://incendiarynetworks.cc/api/v1/license",ORBITFS_APP_VERSION:String(releaseVersion||install.release_version||"unknown"),ORBITFS_ENGINE_RELEASE_PROVIDER:"https://incendiarynetworks.cc/api/v1/updater",ORBITFS_ENGINE_RELEASE_TIMEOUT_MS:"30000",ORBITFS_VERCEL_TIMEOUT_MS:"30000",ORBITFS_LICENSE_REFRESH_MINUTES:"30",ORBITFS_LICENSE_TIMEOUT_MS:"8000",ORBITFS_SCHEMA_VERSION:String(install.schema_version||"1"),ORBITFS_RELEASE_CHANNEL:String(releaseChannel||install.release_channel||"stable"),ORBITFS_PANEL_RELEASE_VERSION:String(releaseVersion||install.release_version||""),ORBITFS_RELEASE_ID:String(releaseId||install.release_id||""),ORBITFS_RELEASE_SHA256:String(releaseSha256||install.release_sha256||""),ORBITFS_RELEASE_SOURCE_COMMIT:String(releaseSourceCommit||install.release_source_commit||"")};for(const [name,value] of Object.entries(vars)){if(value)await upsertVercelEnv(install,name,value)}}
...
export async function deployPanel(install:any,action:DeployAction,version?:string){
  ...
  const result=await masterExecuteDeployment({installationId:install.id,userRef:install.auth_user_id,bindingId:install.license_binding_id,licenseId,releaseId:release.metadata.releaseId,action,actorRef:install.auth_user_id,vercelAccessToken:vercel.token,vercelTeamId:vercel.teamId,vercelProjectId:install.vercel_project_id,vercelProjectName:install.vercel_project_name,env:{SUPABASE_URL:`https://${install.supabase_project_ref}.supabase.co`,SUPABASE_PUBLISHABLE_KEY:publishable,ORBITFS_DB_SECRET:dbSecret}});
